...
public class rest {
    ...
    public String getBooktickets(String pac_id) throws SQLException {
        try {
...
/**
            ...
            Form form =new Form();
            form.param("user_id", "4563");
            form.param("package_id",pac_id);
            Response response = service.path("rest").path("trips").request().post(javax.ws.rs.client.Entity.entity(form,MediaType.APPLICATION_FORM_URLENCODED),Response.class);
            System.out.println("-----------------------------------------------------------------------------------------------");
        return "booked.xhtml";
        }
        catch(Exception e)
